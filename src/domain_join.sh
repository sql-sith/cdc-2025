...
apt install 1> /dev/null ; apt update 1> /dev/null ; apt install 1> /dev/null ; apt upgrade -y 1> /dev/null ; apt autoremove -y 1> /dev/null

echo "Installing packages..."

apt install realmd libnss-sss libpam-sss sssd sssd-tools adcli samba-common-bin oddjob oddjob-mkhomedir packagekit 1> /dev/null
...
cp /etc/sssd.conf /etc/sssd.conf.old
...
echo "Editing pam configuration..."
...
if [ $yes_or_no -eq "y" ] || [ $yes_or_no -eq "Y" ]; then
    ...
    apt install libsss-sudo -y 1> /dev/null
